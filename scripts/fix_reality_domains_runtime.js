...
const marker = `    const endgameModel = buildEndgameModel({
      verifiedOutcomeVolume,
      loopCount,
    });

    const realityDomains = buildRealityDomains();
    const ingestionPriority = buildIngestionPriority();
`;

if (s.includes(marker)) {
  const replacement = `    const endgameModel = buildEndgameModel({
      verifiedOutcomeVolume,
      loopCount,
    });
...
console.log("Reality domain runtime fixed.");
