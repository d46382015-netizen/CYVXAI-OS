...
class IntegrationHub {
  constructor(options = {}) {
    ...
    this.requirements = integrationRequirements(this.env, this.required);
    this.data = options.data || new SupabaseDataClient({ env: this.env, fetch: options.fetch });
    this.identity = options.identity || new IdentityGateway({ env: this.env, fetch: options.fetch, required: this.requirements.identity });
    ...
    this.edge = options.edge || new EdgeGuard({ env: this.env, required: this.requirements.edge });
    this.flags = options.flags || new FeatureFlagService({ env: this.env, dataClient: this.data, required: this.requirements.feature_flags });
    this.queue = options.queue || new SupabaseQueueClient({ env: this.env, dataClient: this.data, required: this.requirements.queue });
    this.email = options.email || new TransactionalEmail({ env: this.env, fetch: options.fetch, required: this.requirements.email });
    this.analytics = options.analytics || new PostHogClient({ env: this.env, fetch: options.fetch, required: this.requirements.product_analytics });
    this.ai = options.ai || new AITrace({ env: this.env, fetch: options.fetch, telemetry: this.telemetry, required: this.requirements.ai_observability });
  ...
  snapshot() {
    ...
    const checks = [
      ...
      check("edge", !requirements.edge || providers.edge.configured, requirements.edge, providers.edge.configured ? "origin-secret" : "missing"),
      check("queue", !requirements.queue || providers.queue.configured, requirements.queue, providers.queue.queue_name),
      check("queue_worker", !requirements.queue || providers.queue_worker.enabled, requirements.queue, providers.queue_worker.enabled ? "enabled" : "disabled"),
      ...
      check("product_analytics", !requirements.product_analytics || (providers.product_analytics.configured && providers.product_analytics.enabled), requirements.product_analytics, providers.product_analytics.enabled ? providers.product_analytics.host : "disabled"),
